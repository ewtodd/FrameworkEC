...
static void hid_startup(void)
{
	...
	MCHP_I2C_CTRL(HID_SLAVE_CTRL) = BIT(7) | BIT(6) | BIT(3) | BIT(0);
}
DECLARE_HOOK(HOOK_CHIPSET_STARTUP, hid_startup, HOOK_PRIO_DEFAULT);
...
static const uint8_t report_desc[] = {
	...
	0x0A, 0xD1, 0xE4, /* USAGE ID (Modified Change Sensitivity Percent of
			     Range) */
	0x15, 0x00, /* LOGICAL_MINIMUN (0x00) */
...
static struct i2c_hid_descriptor hid_desc = {
	...
	.wMaxInputLength = I2C_HID_HEADER_SIZE +
			   sizeof(struct als_input_report), /*Note if there are
							       multiple reports
							       this has to be
							       max*/
	.wOutputRegister = 0,
...
{
	uint16_t newIlluminaceValue =
		*(uint16_t *)host_get_memmap(EC_MEMMAP_ALS);
	...
	if (als_polling_mode_count <= 60) {
		...
		task_set_event(TASK_ID_HID,
			       ((1 << HID_ALS_REPORT_LUX) |
				EVENT_REPORT_ILLUMINANCE_VALUE),
			       0);
	} else {
		if (ABS(als_sensor.illuminanceValue - newIlluminaceValue) >
		    granularity) {
			als_sensor.illuminanceValue = newIlluminaceValue;
			task_set_event(TASK_ID_HID,
				       ((1 << HID_ALS_REPORT_LUX) |
	 ...
	 * at most 1 lux when the ambient light is below 25 lux and a
	 * granularity of at most 4% of the ambient light when it is above 25
	 * lux. This enable the adaptive brightness algorithm to perform smooth
	 * screen brightness transitions.
	 */
...
static int i2c_hid_touchpad_command_process(size_t len, uint8_t *buffer)
{
	...
	case I2C_HID_CMD_GET_REPORT:
		...
		case REPORT_ID_CONSUMER:
			response_len = fill_report(
				buffer, report_id, &consumer_button,
				sizeof(struct consumer_button_report));
			break;
		case REPORT_ID_SENSOR:
			if (report_type == 0x01) {
				response_len = fill_report(
					buffer, report_id, &als_sensor,
					sizeof(struct als_input_report));
			} else if (report_type == 0x03) {
				response_len = fill_report(
					buffer, report_id, &als_feature,
	...
	case I2C_HID_CMD_SET_REPORT:
		switch (report_id) {
		case REPORT_ID_SENSOR:
			extract_report(len, buffer, &als_feature,
				       sizeof(struct als_feature_report));
			break;
		default:
			break;
...
int i2c_hid_process(unsigned int len, uint8_t *buffer)
{
	...
	case I2C_HID_INPUT_REPORT_REGISTER:
		...
		if (input_mode == REPORT_ID_RADIO) {
			response_len = fill_report(buffer, REPORT_ID_RADIO,
						   &radio_button,
						   sizeof(struct radio_report));
		} else if (input_mode == REPORT_ID_CONSUMER) {
			response_len = fill_report(
				buffer, REPORT_ID_CONSUMER, &consumer_button,
				sizeof(struct consumer_button_report));
		} else if (input_mode == REPORT_ID_SENSOR) {
			response_len = fill_report(
				buffer, REPORT_ID_SENSOR, &als_sensor,
				sizeof(struct als_input_report));
		}
		break;
...
void hid_irq_to_host(void)
{
	...
	while (((MCHP_I2C_STATUS(HID_SLAVE_CTRL) & BIT(0)) == 0) &&
	       ++timeout < 1000) {
		usleep(10);
	}
	/*Deassert interrupt */
...
void hid_handler_task(void *p)
{
	...
	while (1) {
		...
		if (event & EVENT_REPORT_ILLUMINANCE_VALUE) {
			...
			hook_call_deferred(&report_illuminance_value_data,
					   ((int)als_feature.report_interval) *
						   MSEC);
		}
				...
				if ((event & 0xBFFF) & (1 << i)) {
					...
					case HID_KEY_DISPLAY_BRIGHTNESS_UP:
						...
						if (key_states[i]) {
							consumer_button
								.button_id =
								BUTTON_ID_BRIGHTNESS_INCREMENT;
						} else {
							consumer_button
								.button_id = 0;
						}
					...
					case HID_KEY_DISPLAY_BRIGHTNESS_DN:
						...
						if (key_states[i]) {
							consumer_button
								.button_id =
								BUTTON_ID_BRIGHTNESS_DECREMENT;
						} else {
							consumer_button
								.button_id = 0;
						}
					...
					/* we don't need to assert the interrupt
					 * when system state in S0ix */
